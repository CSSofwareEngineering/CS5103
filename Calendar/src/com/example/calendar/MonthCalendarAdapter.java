...
public class MonthCalendarAdapter extends BaseAdapter {
	...
	int currentMonthFirstDayOfWeek, intCurrentMonth, intSelectedPosition;

	Context viewContext;
	CalendarPreferences calendarPreferences;
	MonthSectionFragment monthFragment;
	
	public MonthCalendarAdapter(Context viewContext, Calendar currentMonth, MonthSectionFragment fragment) {
		this.currentMonth = currentMonth;
		this.viewContext = viewContext;
		this.dayStringList = new ArrayList<String>();
		this.monthFragment = fragment;
		...
		countCalendar.set(Calendar.DAY_OF_MONTH, 1);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final int intSelectedDay = Integer.parseInt(strSelectedDate.substring(0,2));
		int intSelectedMonth = selectedDay.get(Calendar.MONTH);
		...
		parent.setBackgroundColor(parent.getContext().getResources().getColor(R.color.black));
		...
		if ((intSelectedDay == intListDay) & (intSelectedMonth == intListMonth))
	...
	public void setSelectedDate(Calendar selectedDay) {
		this.selectedDay = (Calendar)selectedDay.clone();
	}
